...
const SearchbarByLocation = ({userLocation}) => {
    ...
    useEffect(() => {
        axios.get(`${import.meta.env.VITE_LIVE_URL}division`)
            .then(res => setDivisionData(res.data))
    }, [])
    useEffect(() => {
        const division = divisionData.find(item => item?.name === selectedOption1?.value)
        console.log(division)
        axios.get(`${import.meta.env.VITE_LIVE_URL}districts?data=${division?.id}`)
            .then(res => setDistrictsData(res.data))
    }, [divisionData, selectedOption1])
    useEffect(() => {
        const district = districtsData.find(item => item?.name === selectedOption2?.value)
        axios.get(`${import.meta.env.VITE_LIVE_URL}upazila?data=${district?.id}`)

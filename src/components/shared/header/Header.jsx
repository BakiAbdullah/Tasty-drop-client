...

            {
                searchOpen && <div className='absolute top-0 left-0  w-full bg-white text-center py-6 z-10 flex justify-center items-center '  >
                    <div className='relative w-[70%]  md:w-[60%] flex items-center  '>
                        <input className='outline-none border-2 w-full py-2 px-3 border-black pe-10 md:pe-14' type="text" placeholder='Search' /> <AiOutlineSearch className='absolute top-[10px] right-14' size={26} />
                        <span onClick={() => setSearchOpen(false)} className='ms-4 cursor-pointer '><AiOutlineClose size={23} /></span>
                    </div>
                </div>
            }

            <div className='flex justify-between items-center  bg-white   '>
                <div onClick={() => setIsOpen(!isOpen)} className='bolck md:hidden ps-4' >
                    {
                        isOpen ? <AiOutlineMenu size={26} /> : <AiOutlineClose size={26} />
                    }
                </div>
